...
machaub_sudos = macha_mongodb["sudos_db"]
...
async def add_sudo(sudo_id):
    ...
    sudo_db = await machaub_sudos.find_one({"_id": "SUDO_USERS"})
    if sudo_db:
        await machaub_sudos.update_one({"_id": "SUDO_USERS"}, {"$push": {"sudo_id": isudo_id}})
    else:
        isudo_id = [isudo_id]
        await machaub_sudos.insert_one({"_id": "SUDO_USERS", "sudo_id": isudo_id})

# Get sudo users from database
async def get_sudos():
    s_u_i = await machaub_sudos.find_one({"_id": "SUDO_USERS"})
    if s_u_i:
...
async def remove_sudo(sudo_id):
    r_sudo_id = await machaub_sudos.find_one({"_id": "SUDO_USERS"})
    irudo_id = int(sudo_id)
    if r_sudo_id:
        await machaub_sudos.update_one({"_id": "SUDO_USERS"}, {"$pull": {"sudo_id": irudo_id}})
    else:
        return False

# Check if user already in sudo databse
async def check_if_sudo(sudo_id):
    already_sudo = await machaub_sudos.find_one({"_id": "SUDO_USERS"})
    if already_sudo:
