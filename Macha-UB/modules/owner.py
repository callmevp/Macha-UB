...
# To Block a user
@machaub_on_cmd(command="block", modlue=mod_file, no_sudos=True)
async def block_dumb(_, message: Message):
  shit_id = message.chat.id
  r_msg = message.reply_to_message
  gonna_block_u = await e_or_r(machaub_message=message, msg_text="`Blocking User...`")
  try:
    if r_msg:
      await MACHAUB.block_user(r_msg.from_user.id)
      await gonna_block_u.edit("`Successfully Blocked This User`")
    else:
      await MACHAUB.block_user(shit_id)
      await gonna_block_u.edit("`Successfully Blocked This User`")
  except Exception as lol:
    await gonna_block_u.edit(f"**Error:** `{lol}`")

# To Unblock User That Already Blocked
@machaub_on_cmd(command="unblock", modlue=mod_file, no_sudos=True)
async def unblock_boi(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await e_or_r(machaub_message=message, msg_text="`Unblocking User...`")
  try:
    await MACHAUB.unblock_user(good_bro)
    await gonna_unblock_u.edit(f"`Successfully Unblocked The User` \n**User ID:** `{good_bro}`")
  except Exception as lol:
    await gonna_unblock_u.edit(f"**Error:** `{lol}`")

# Leave From a Chat
@machaub_on_cmd(command="kickme", modlue=mod_file, no_sudos=True, only_groups=True)
async def ubkickme(_, message: Message):
  i_go_away = await e_or_r(machaub_message=message, msg_text="`Leaving This Chat...`")
  try:
    await MACHAUB.leave_chat(message.chat.id)
...
@machaub_on_cmd(command="chats", modlue=mod_file, no_sudos=True)
async def ubgetchats(_, message: Message):
  total=0
  getting_chats = await e_or_r(machaub_message=message, msg_text="`Checking Your Chats, Hang On...`")
  async for dialog in MACHAUB.iter_dialogs():
    try:
      await MACHAUB.get_dialogs_count()
      total = total+1
