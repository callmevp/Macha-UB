...
UPSTREAM_REPO_URL = "https://github.com/callmevp/Macha-UB"
...
@machaub_on_cmd(command="update", modlue=mod_file)
async def upstream(client, message):
    status = await e_or_r(machaub_message=message, msg_text=f"`Checking For Updates from` [Macha-Userbot]({UPSTREAM_REPO_URL}) `Repo...`")
    ...
    if ac_br != "master":
        await status.edit(
            f"`Can't update your Macha-Userbot becuase you're using a custom branch.` \n\n**Default Branch:** `master` \n**You are on:** `{ac_br}` \n`Please change to master branch.`"
        )
        repo.__del__()
        return
    try:
    ...
    if "now" not in conf:
        try:
            ...
            pass
        if changelog:
            changelog_str = f"**New Updates available for Macha Userbot** \n\n**Branch:** [{ac_br}]({UPSTREAM_REPO_URL}/tree/{ac_br}) \n\n**Changelog (last 10):** \n\n{changelog}"
            if len(changelog_str) > 4096:
                await status.edit("`Changelog is too big, view the file to see it.`")
                file = open("MACHAUB_git_commit_log.txt", "w+")
                file.write(changelog_str)
                file.close()
                await MACHAUB.send_document(
                    message.chat.id,
                    "MACHAUB_git_commit_log.txt",
                    caption=f"Do `{Config.CMD_PREFIX}update now` to update your Macha-Userbot",
                    reply_to_message_id=status.message_id,
                )
                remove("MACHAUB_git_commit_log.txt")
            else:
                return await status.edit(
                    f"{changelog_str}\n\nDo `.update now` to update your Macha-Userbot",
                    disable_web_page_preview=True,
                )
        else:
            await status.edit(
                f"\n**Macha-Userbot is Uptodate with** [{ac_br}]({UPSTREAM_REPO_URL}/tree/{ac_br})**\n",
                disable_web_page_preview=True,
            )
            repo.__del__()
        return
    if Config.HEROKU_API_KEY is not None:
        ...
        await status.edit("`Successfully Updated!` \n**Restarting Now...**",)
        args = [sys.executable, "-m" "Macha-UB"]
        execle(sys.executable, *args, environ)
        return

# Userbot restart module
async def restart_machaub():
    if Config.HEROKU_API_KEY is not None:
...
@machaub_on_cmd(command="restart", modlue=mod_file)
async def restart(client, message):
    restart_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
    await restart_msg.edit("`Macha-Userbot is restarting! Please wait...`")
    try:
        await restart_machaub()
    except Exception as e:
        await restart_msg.edit(f"**Error:** `{e}`")


@machaub_on_cmd(command="logs", modlue=mod_file)
async def log(client, message):
    try:
        await e_or_r(machaub_message=message, msg_text="`Getting Logs`")
        heroku_conn = heroku3.from_key(Config.HEROKU_API_KEY)
        ...
        if len(f_logs) > 4096:
            ...
            file.write(f_logs)
            file.close()
            await MACHAUB.send_document(message.chat.id, "logs.txt", caption=f"Logs of `{Config.HEROKU_APP_NAME}`")
            remove("logs.txt")
    except Exception as e:
