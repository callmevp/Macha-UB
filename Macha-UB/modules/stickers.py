...
from Macha-UB.helpers.pyrogram_help import get_arg, convert_to_image
...
@machaub_on_cmd(command="packinfo", modlue=mod_file)
async def packinfo(client, message):
    pablo = await e_or_r(machaub_message=message, msg_text="Processing...")
...
@machaub_on_cmd(command="kang", modlue=mod_file)
async def kang_stick(client, message):
    pablo = await e_or_r(machaub_message=message, msg_text="`Kanging This Sticker to My Pack...`")
    if not message.reply_to_message:
        await pablo.edit("`Please Reply to a Sticker!`")
        return
    ...
    packshortname = f"MACHAUB_{message.from_user.id}_{pack}"
    non = [None, "None"]
    emoji = "🤔"
    try:
        Hell = Hell.strip()
    ...
    if exist:
        ...
        await pablo.edit("**Sticker Kanged!** \n\n**Emoji:** {} \n**Pack:** [Here](https://t.me/addstickers/{})".format(emoji, packshortname))
        try:
            if os.path.exists("Kanged_Sticker_MACHAUB.png"):
                os.remove("Kanged_Sticker_MACHAUB.png")
            downname = "./downloads"
...
def resize_image(image):
    ...
    file_name = "Kanged_Sticker_MACHAUB.png"
