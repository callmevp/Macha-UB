...
**Example:**
  ✘ `duck_s`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}duck_s Macha Userbot`
  ✘ `google`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}google Macha Userbot`
...
@machaub_on_cmd(command="duck_s", modlue=mod_file)
async def duckduckg_s(client, message):
    pablo = await e_or_r(machaub_message=message, msg_text="`Searcing in DuckDuckGo...`")
...
@machaub_on_cmd(command="google", modlue=mod_file)
async def google_s(client, message):
    pablo = await e_or_r(machaub_message=message, msg_text="`Searching in Google...`")
    query = get_arg(message)
