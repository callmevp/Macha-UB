...
from Macha-UB.core.main_cmd import machaub_on_cmd, e_or_r, machaub_on_cf, SUDO_IDS
from Macha-UB.helpers.pyrogram_help import get_arg
from Macha-UB.core.machaub_database.machaub_db_gbans import gban_usr, get_gbanned, get_gban_reason, ungban_usr
from Macha-UB.helpers.pyrogram_help import get_ma_chats
...
@machaub_on_cmd(command="gban", modlue=mod_file)
async def me_goin_oflin(_, message: Message):
    gban_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
    machaub_devs = [1154859827]
    r_msg = message.reply_to_message
    gban_rsn = get_arg(message)
    machaub_owner = await MACHAUB.get_me()
    if r_msg:
        if gban_rsn:
            gban_rson = gban_rsn
        else:
    ...
    for gokid in f_chats:
        ub_failed = 0
        try:
            await MACHUB.kick_chat_member(chat_id=gokid, user_id=int(gban_uid))
        except:
            ub_failed += 1
...
@machaub_on_cmd(command="ungban", modlue=mod_file)
async def me_goin_oflin(_, message: Message):
    ungban_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
    ...
    if gban_usr:
        if gban_usr.isnumeric():
            ...
            if "@" in gban_usr:
                ...
                usr_name = gban_usr
            get_him = await MACHAUB.get_users(usr_name)
    ...
    for good_boi in ung_chats:
        ub_failed = 0
        try:
            await MACHAUB.unban_chat_member(chat_id=good_boi, user_id=ungban_uid)
        except:
            ub_failed += 1
...
@machaub_on_cmd(command="gbans", modlue=mod_file)
async def me_goin_oflin(_, message: Message):
    glist_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
    ...
    if len(gban_txt) > 4096:
        file = open("MACHAUB_Gban_List.txt", "w+")
        file.write(gban_txt)
        file.close()
        await MACHAUB.send_document(
            message.chat.id,
            "MACHAUB_Gban_List.txt",
            caption=f"Gban List"
        )
        os.remove("MACHAUB_Gban_List.txt")
    else:
        await glist_msg.edit(gban_txt)


@machaub_on_cf(filters.incoming & ~filters.me & ~filters.user(SUDO_IDS))
async def gbanner(_m, message: Message):
    ...
    if is_gbanned:
        if message.chat.type == "private":
            await MACHAUB.block_user(gbanned_usr_id)
        else:
            try:
                await MACHAUB.kick_chat_member(chat_id=gban_chat_id, user_id=gbanned_usr_id)
            except:
                return
        await MACHAUB.send_message(chat_id=gban_chat_id, text=f"**#GBAN_DB** \n`Gbanned User Joined to this chat. So I've Banned Him!` \n\n**User ID:** `{gbanned_usr_id}` \n**Reason:** `{is_gbanned}`")
    else:
        return
