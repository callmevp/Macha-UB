...
from Macha-UB.helpers.up_to_tg import guess_and_send
...
# Download path
megadir = "./MachaUb/Megatools"

# Run bash cmd in python
def macha_mega_runner(command):
...
def split_files(input_file, out_base_path):
    ...
    macha_fs.split(file=split_file, split_size=split_fsize, output_dir=out_path)

@machaub_on_cmd(command="megadl", modlue=mod_file)
async def megatoolsdl(_, message: Message):
    megatools_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
    url = message.text
    ...
    await loop.run_in_executor(None, partial(macha_mega_runner, megacmd))
    machaub_path_f = f"{os.getcwd()}/MachaUb/Megatools/{str(message.from_user.id)}"
    folder_f = [val for sublist in [[os.path.join(i[0], j) for j in i[2]] for i in os.walk(machaub_path_f)] for val in sublist]
    print(folder_f)
    await megatools_msg.edit("`Downloading Finished! Trying to upload now`")
    try:
        for macha_m in folder_f:
            file_size = os.stat(macha_m).st_size
            if file_size > 2040108421:
                split_out_dir = machaub_path_f + "splitted_files"
                await megatools_msg.edit("`Large File Detected, Trying to split it!`")
                loop = get_running_loop()
                await loop.run_in_executor(None, partial(split_files(input_file=macha_m, out_base_path=split_out_dir)))
                await megatools_msg.edit("`Splitting Finished! Uploading Now...`")
                for splitted_f in split_out_dir:
                    await MACHAUB.send_document(chat_id=message.chat.id, document=splitted_f, caption=f"`Uploaded by` {(await MACHAUB.get_me()).mention}")
            else:
                chat_id = message.chat.id
                await guess_and_send(input_file=macha_m, chat_id=chat_id, thumb_path=machaub_path_f)
        await megatools_msg.edit("`Uploading Finished!`")
    except Exception as e:
        await megatools_msg.edit(f"**Error:** `{e}`")
    try:
        shutil.rmtree(machaub_path_f)
    except Exception as e:
        await megatools_msg.edit(f"**Error:** `{e}`")
