...
from Macha-UB.core.machaub_database.machaub_db_afk import me_afk, get_afk, me_online
from Macha-UB.core.main_cmd import machaub_on_cmd, e_or_r, machaub_on_cf
...
@machaub_on_cmd(command="afk", modlue=mod_file)
async def me_goin_oflin(_, message: Message):
    afk_msg = await e_or_r(machaub_message=message, msg_text="`Processing...`")
...
async def back_online_bois(_, message: Message):
    ...
    await me_online()
    await e_or_r(machaub_message=message, msg_text=f"**I'm No Longer AFK** \n\n**Afk Time:** `{afk_time}` \n**Reason:** `{a_reason}`")
